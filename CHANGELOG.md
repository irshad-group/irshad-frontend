...
- **A map of the office, on the directorate page.** The address panel now carries a small map centred on the exact coordinates, with a pin, that opens the reader's own maps application when clicked. Built from ordinary tile images on the server rather than a canvas map, so it is in the HTML, works with JavaScript disabled, and adds no map library to the page.
