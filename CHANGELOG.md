...
- `pocketbase/seed/import-geography.mjs` — upserts that dataset by slug and never deletes, so the development seed's directorates keep the procedures already attached to them. Dropped connections and 5xx responses are retried with backoff: the import is ~2,200 records and, with photos, several hundred megabytes, and the far side will close a socket at some point during a run that long.
